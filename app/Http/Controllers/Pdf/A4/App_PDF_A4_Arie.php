...
class App_PDF_A4_Arie
{

    public function generar($data)
    {
        # Detalles generales del documento
        $pdf = new App_PDF_A4_Arie_HF('P', 'mm', 'A4', true, 'UTF-8', false, false, $data);
        $pdf->setPrintFooter(false);
        $pdf->SetTopMargin(74.594446);
        $pdf->SetLeftMargin(5);
        ...
        $pdf->SetFont('helvetica', 'B', 8);
        $pdf->MultiCell(100, 0, 'Consulta tu comprobante en nuestra web ' . $_ENV['URL.FRONT-OFFICE'], '', 'C');
        ...
        if (isset($data['usuario'])) {
            $pdf->MultiCell(68, 0, 'USUARIO: ' . $data['usuario'], '', 'L');
        }
        if (isset($data['sf'])) {
            $pdf->MultiCell(68, 0, 'SF: ' . $data['sf'], '', 'L');
        }
        if (isset($data['caja'])) {
            $pdf->MultiCell(68, 0, 'CAJA: ' . $data['caja'], '', 'L');
        }

        $pdf->SetXY($x + 68, $y);
        if (isset($data['s_caja'])) {
            $pdf->MultiCell(68, 0, 'S-CAJA: ' . $data['s_caja'], '', 'L');
        }
